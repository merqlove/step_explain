...
function StepData(step) {
  ...
  if (NotNullEmpty(secondChildren)) {
    ...
    if (firstChildren.nodeName === 'H3') {
      ...
      var image = step.querySelector('img');
...
function Renderer(opts, data, i, cb) {
  (0, _ejs.renderFile)(opts.template, data, {}, function (err2, str) {
    ...
    if (data.header === true) {
      var newOpts = opts;
      var index = newOpts.headerNumber < 10 ? "0".concat(newOpts.headerNumber) : newOpts.headerNumber;
      newOpts.destTitle = "".concat(newOpts.dest, "/").concat(index, ". ").concat(data.title);
      ...
      var _index = i < 10 ? "0".concat(i + 1) : i + 1;

      var fileName = "page_".concat(_index, ".html");
